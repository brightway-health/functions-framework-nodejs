...
const ce_to_legacy_event_1 = require("../../src/middleware/ce_to_legacy_event");
const cloudevents_1 = require("../../src/cloudevents");
const ceHeaders = (eventType, source) => ({
    'ce-id': 'my-id',
    'ce-type': eventType,
    'ce-source': source,
    'ce-specversion': '1.0',
...
describe('parseSource', () => {
    ...
    testData.forEach(testCase => {
        it(testCase.name, () => {
            const { service, name } = ce_to_legacy_event_1.parseSource(testCase.source);
    ...
    it('throws an exception on invalid input', () => {
        assert.throws(() => ce_to_legacy_event_1.parseSource('invalid'), cloudevents_1.EventConversionError);
    });
});
describe('ceToLegacyEventMiddleware', () => {
    const testData = [
        ...
        },
        {
            ...
            headers: ceHeaders('google.firebase.database.document.v1.written', '//firebasedatabase.googleapis.com/projects/_/instances/my-project-id'),
            body: { some: 'value' },
            expectedBody: {
                context: {
                    eventId: 'my-id',
        ...
        {
            name: 'PubSub request',
            headers: ceHeaders('google.cloud.pubsub.topic.v1.messagePublished', '//pubsub.googleapis.com/projects/sample-project/topics/gcf-test'),
            body: {
                message: {
                    data: 'value',
                },
        ...
        },
        {
            ...
            headers: ceHeaders('google.cloud.storage.object.v1.finalized', '//storage.googleapis.com/projects/_/buckets/some-bucket'),
            body: {
                some: 'value',
                kind: 'storage#object',
            },
        ...
        },
        {
            ...
            headers: ceHeaders('google.firebase.auth.user.v1.created', '//firebaseauth.googleapis.com/projects/my-project-id'),
            body: {
                metadata: {
    ...
    testData.forEach(test => {
        it(test.name, () => {
            ...
            const request = {
                ...
                header: (key) => test.headers[key],
            };
            ce_to_legacy_event_1.ceToLegacyEventMiddleware(request, {}, next);
